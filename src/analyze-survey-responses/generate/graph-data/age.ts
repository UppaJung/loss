...
import { ScenarioLabelSurveyKeyPairs } from "../../decode-questions/scenario-labels.ts";
...
export const getAgeScenariosMatchedScatterPlotData = (path: string, responses: AugmentedSurveyResponses) => {
	...
	const data: {x: number; y: number}[] = responses.map( (response) => {
		...
		const scenariosMatched = ScenarioLabelSurveyKeyPairs.reduce( (result, [_, surveyKey]) => {
